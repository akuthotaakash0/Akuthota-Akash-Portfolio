document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typedTarget) {
    ...
    const typeTitle = () => {
      ...
      if (!deleting && charIndex < current.length) { charIndex += 1; setTimeout(typeTitle, 75); return; }
      if (!deleting) { deleting = true; setTimeout(typeTitle, 1800); return; }
      if (charIndex > 0) { charIndex -= 1; setTimeout(typeTitle, 45); return; }
      deleting = false;
  ...
  } else reveals.forEach((section) => section.classList.add('visible'));
  ...
  if (scrollTop) {
    const updateScrollButton = () => scrollTop.classList.toggle('show', window.scrollY > 500);
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (!response.ok || data.success === false) throw new Error(data.message || 'Unable to send the message.');
        contactForm.reset();
        if (formNote) { formNote.textContent = `Message sent successfully${name ? `, ${name}` : ''}! Akash will receive it by email.`; formNote.classList.add('success'); }
      } catch (error) {
        if (formNote) { formNote.textContent = 'We could not send the message right now. Please try again in a moment or use the email button beside the form.'; formNote.classList.remove('success'); }
      } finally {
